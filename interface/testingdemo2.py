...
class RoverControl(QtWidgets.QMainWindow):
    ...
    def __init__(self, node):
        ...
        # --- *** NEW: Start the map widget *** ---
        self.map_cam.start(self.streams[5])
    ...
    def setup_ui(self):
        ...
        self.btn_cap.setStyleSheet("padding:6px; font-size:11px;")
        ...
        # --- *** NEW: Add RTAB-Map Widget *** ---
        self.map_cam = CameraWidget(
            "RTAB-Map",
            self.node,
            self.ros_frame_ready
        )
        # Give it a size that fits the right panel
        self.map_cam.display.setFixedSize(330, 250) 
        self.map_cam.display.setStyleSheet(
            "background:#000; border:1px solid #444; color:white; font-size:12px;"
        )
        # Add it to the top of the right panel
        right.addWidget(self.map_cam)
        # --- *** END NEW *** ---
